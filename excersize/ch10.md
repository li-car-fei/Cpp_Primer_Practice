...
> 因为它们实现了共同的操作，所以称为算法，而泛型指的是它们可以操作在多种容器类型上。泛型算法本身不执行容器操作，只是单独依赖迭代器和迭代器操作实现。算法可能改变容器内保存元素的值，可能移动元素，但是不能直接添加或删除元素。

> lambda 表达式，向算法传递函数。lambda 表达式表示一个可调用的代码单元，可以理解成是一个未命名的内联函数：`[capture list](parameter list) -> return type {function body}`

- capture type: 从上层函数捕获的局部变量，不在这捕获无法访问,不可忽略`[]`
- parameter list: 函数的参数列表，可忽略。通常是遍历的元素
- return type: 函数返回类型，可忽略
- function body: 函数体

拷贝算法：`copy(list.begin(), list.end(), back_inserter(vec));`
从两个 iter 之间，根据 inserter 的类型，将元素拷贝到新容器 vec 中；

- inserter(): 用 insert 进行元素插入
- back_inserter(): 用 push_back()进行元素插入
- front_inserter(): 用 push_front()进行元素插入
...
accumulate 是对于 int 类型的算法，如果在 double 上使用，存在类型转换(1+0+3)
...
`C 风格字符串是用指向字符的指针表示的`，因此会比较两个`指针`的值（地址），而不会比较这两个字符串的内容。
...
(a) vector<int> vec; list<int> lst; int i;
	while (cin >> i)
		lst.push_back(i);
    // copy时应该保证目标序列至少包含与输入序列一样多的元素(size)
    // 需要 vec.resize(lst.size()
	copy(lst.cbegin(), lst.cend(), vec.begin());
(b) vector<int> vec;
	vec.reserve(10);            // 分配了capacity，但不会新建元素
    // 改为 vec.resize(10)  改变size，按默认新建元素
...
- (b) reserve 分配了足够的空间(capacity)，但是不会创建新元素。算法可能改变容器中保存的元素的值，也可能在容器内移动元素，永远不会直接添加和删除元素(c++ priemr 中文版第五版 P338)，所以此处应该改为 resize(10)。
...
auto eliminate_duplicates(std::vector<std::string> &vs) -> std::vector<std::string>&
{
    ...
    println(vs);        // a a a a s v v

    // 将容器去重，不重复的排在前面，重复的排在后面
    // 容器的size不会变，重复的有一个，然后其他为空
    // 返回的 new_end 指向 最后一个不重复元素的后面
    auto new_end = std::unique(vs.begin(), vs.end());
    println(vs);        // a s v a   v
...
void count(){
    int i = 10;
    auto myLambda = [&i]() -> { return i==0 ? true : !(i--) };
    while(!myLambda()) cout << i << endl;
}
```
...
int main()
{
    ...
    // std::placeholders::_1 占位符，调用绑定函数时，会被实际参数替代
    // 遍历时，占位符会被当前遍历的元素值替代
    // _1 指定的第一个参数，是指实际调用的第一个参数，不是isLesserThanOrEqualTo6 中的第一个参数
...
// bool check_size(const int &i, const int sz){
//     return i > sz ? true : false;
// }

// auto check_size = [&sz](const int &i) -> bool {eturn i > sz ? true : false;}

int main(){
    vector<int> vec = {1,2,3,4};
    string str = "he";
    sz = str.size()
    // cout << vector.find_if(vec.begin(), vec.end(), bind(check_size, _1, str.size())) << endl;

    cout << vector.find_if(vec.begin(), vec.end(), [&sz](const int &i)->{return i > sz ? true : false;}) << endl;
}
```
...
int main()
{
    ...
    // std::placeholders::_1 占位的是实际遍历时的第一个参数，即int
    // bind函数，通用函数适配器，为函数调用时绑定参数
...
{
    std::sort(vs.begin(), vs.end());
    // unique进行去重，将重复的放在后面，不重复的放在前面
    // 容器的size不改变，返回指向最后一个不重复元素的后一位的iter
...
void biggies(vector<string> &words, vector<string>::size_type sz)
{
    ...
    // for_each 是标准函数，从一个头尾指向的序列中运行 函数
...
int main()
{
    ...
    // copy：前两个参数指定拷贝输入范围，第三个指向目标序列（拷贝到哪）

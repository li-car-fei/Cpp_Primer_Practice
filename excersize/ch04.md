...
```cpp
(a) *iter++;		// 取 *iter ,然后 iter加一
(b) (*iter)++;		// string 没有 ++
(c) *iter.empty();	// 要有括号 *(iter).empty()
(d) iter->empty();  // *(iter).empty()
(e) ++*iter;		// string 没有 ++
(f) iter++->empty();	// *(iter).empty() 然后 iter加一
```
...
```cpp
int x[10];   int *p = x; // x为数组的首地址，将其赋给p指针，不用取址符&

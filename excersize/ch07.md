...
看基础：https://www.runoob.com/cplusplus/cpp-classes-objects.html

拷贝函数：
当类内有指针时，一定要自己写拷贝函数，进行 new 的拷贝，用编译器自带的会用把地址拷贝给变量，导致互相影响。
...
如果某个成员只能在内部和子类中访问，那么它该定义为`protected`;
...
`class` 的成员默认是`private`, 继承默认是`private`继承；
`struct` 的成员默认是`public`, 继承默认是`public`继承；
...
指定一个外部函数或者一个类，该外部函数或者类，能够访问当前类的私有元素。当其他类或者函数想要访问当前类的私有变量时，这个时候应该用友元。
...
class Person {
    ...
    // Person &person 与 const Person &person 的区别在于
    // 后者指定了此引用不能修改引用的值，不能修改赋值等
...
{
    cursor = r*width + c;
    // 如果返回值设定是 Screen 而不是 Screen&
    // 则会返回 *this 的一个副本，导致后续无法继续操作当前 Screen
    return *this;
}
...
public:
    ...
    // 如果构造函数只接受了一个实参，则实际上定义了转换为此类类型的隐式转换
    // 这种构造函数有叫做转换构造函数，编译器只会自动将该参数进行类型转换
    // 需要 explicit 关键字对隐式转换加以阻止
...
`rate`应该是一个**常量表达式**（只读区）。而类内只能初始化整型类型的静态常量（可读写区），所以不能在类内初始化`vec`。修改后如下：

```cpp
// example.h
class Example {
public:
    // 正常情况下，静态变量的初始化要在类外通过 class::variable 实现
    // 想要在类内部初始化，需要通过constexpr指定在编译时求常量

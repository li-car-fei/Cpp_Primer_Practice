...
vector<int> v1;
const vector<int> v2;
// v2 是常量，而也是头部，所以v2.begin()是常量迭代器
...
// 从容器创建拷贝的构造函数，需要容器和元素类型都对得上
// 从两个迭代器创建拷贝的构造函数，只需要元素类型可以相互转换即可
...
先将 list<int> 通过两个迭代器拷贝创建为新的 vector<int> 后再比较
...
- `c1`和`c2`必须是相同类型的容器，并且保存的元素类型也相同
- 元素要支持关系运算符
...
int main(){
    ...
    for(; cin >> str; myDeque.push_back());
    for(auto p = myDeque.cbegin(); p!=myDeque.cend(); ++p){
        cout << *p << endl;
    };
}
...
```cpp
// 先算出数组的长度
int length = sizeof(ia)/size(ia[0]);
// 数组的变量（指针）可以视为迭代器，+length 即是尾部；
vector<int> vec(ia, ia + length);
...
```cpp
iter = vi.insert(iter, *iter++);    // 参数的求值顺序是未知的
```
...
// .reserve() 分配存储空间，分配capacity
svec.reserve(1024);
string word;
while (cin >> word)
	svec.push_back(word);

// .resize() 元素数量，如果小于目前则舍弃部分，大于目前则默认初始化
// capacity（当前总共可以存多少）不会变，只是size（当前存了多少）变了
...
使用 `reserve(100)` 函数预先分配 100 个元素的空间（capacity）。
...
auto replace_with(string &s, string const& oldVal, string const& newVal)
{
    for (size_t pos = 0; pos <= s.size() - oldVal.size();)
        // s[pos] == oldVal[0] 是为了优化，如果首字符不匹配则不需要截取
        if (s[pos] == oldVal[0] && s.substr(pos, oldVal.size()) == oldVal)
...
string name(const string &o_name, const string &pre, const string &after,){
    o_name.append(after.begin(), after.end());// append 直接插入后缀
    return o_name.insert(0, pre.begin(), pre.end());
}
```
...
{
    name.insert(0, pre);            // 往位置0前面插入
    name.insert(name.size(), su);   // 往size位置即末尾前面插入
    return name;
}
...
int main()
{
    ...
    // find_first_of 的第一个参数是寻找的字符串，第二个参数是开始位置
    ...
    // find_first_not_of 的第一个参数是寻找的字符串，第二个参数是开始位置
...
返回 `string::npos`
tips: string 的搜索操作都返回一个 string::size_type 的值，表示匹配发生位置的下标。如果搜索失败则返回一个名为 string::npos 的 static 成员。
...
class VecString{
    public:
        VecString(vector<string>: vec1): vec(vec1)
        cal_sum(const string &type){
            int sum =0;
            switch type:
                case string("int"):
...
int main()
{
    ...
    // replace()方法替换字符，前两个参数指定位置，后面参数为替换目标

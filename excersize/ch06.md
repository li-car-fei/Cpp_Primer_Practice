...
- Lambda 表达式
  （1）Lambda 表达式是匿名的，不需要为其定义一个独立的函数名，可以直接在需要的地方定义和使用。普通函数需要先定义函数名，然后才能调用。
  （2）捕获列表：Lambda 表达式可以指定一个捕获列表，用于在函数体中访问外部变量。捕获列表可以为空，也可以使用值捕获、引用捕获。普通函数无法直接访问外部变量，需要通过参数传递。
  （3）可调用对象：Lambda 表达式生成一个可调用的函数对象，可以像函数一样调用。普通函数是通过函数名调用的。
  （4）内联性：Lambda 表达式通常被编译器内联展开，可以避免函数调用的开销。普通函数的内联展开需要依赖编译器的优化策略和函数定义的位置。
  （5）类型推导：Lambda 表达式可以进行自动类型推导，可以根据上下文推断参数类型和返回类型。普通函数需要显式指定参数类型和返回类型。
  （6）函数重载：普通函数可以进行函数重载，即在同一作用域内可以定义多个同名函数，但参数类型或参数个数不同。Lambda 表达式没有函数名，无法进行函数重载。
...
```cpp
(a) int calc(int, int);             // 只能传入副本参数且可变，下面的不可变
	int calc(const int, const int); // 顶层const不影响传入对象，无法区分
    // 这里无法区分的原因是无法从形参中区分两个函数，无法重载，但两者有区别的
(b) int get();
	double get();                   // 返回值类型与重载无关
(c) int *reset(int *);
	double *reset(double *);
```
...
(b) char *init(int ht = 24, int wd, char bckgrnd);  // 默认参数在后面
```
...
```cpp
(a) inline bool eq(const BigInt&, const BigInt&) {...}  // 内联函数
(b) void putValues(int *arr, int size);         // 声明
```
...
“constxpre” 函数的返回值以及所有形参都应该是字面量类型；
...
```cpp
(a) int calc(int&, int&);               // 只能引用形式输入，非常量
	int calc(const int&, const int&);   // 可以输入临时变量，常量/非常量
(b) int calc(char*, char*);
	int calc(const char*, const char*); // 可以输入临时变量
(c) int calc(char*, char*);
	int calc(char* const, char* const); // 无法区分重参，无法重载
```
...
// decltype(func)* 推断的是一个 指向 返回类型为int的函数 的指针
// 因此vector中元素的类型都是函数指针类型
```
